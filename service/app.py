...
DEFAULT_GPU_ID = int(os.getenv("DEFAULT_GPU_ID", "0"))

app = FastAPI(title="Result1 Inference API", version="0.1.0")
...
def _load_models(device: str) -> None:
    if _state["qwen_model"] is not None and _state["zimage_pipe"] is not None and _state["device"] == device:
        return
...
def _generate_caption(image: Image.Image, user_input: str, min_pixels: int, max_pixels: int, max_new_tokens: int) -> str:
    ...
    inputs = {k: v.to(_state["device"]) if hasattr(v, "to") else v for k, v in inputs.items()}
...
def _run_zimage(
    ...
    gen = None
    if seed is not None:
        gen = torch.Generator(device=_state["device"]).manual_seed(seed)
...
def infer_result1(
    ...
    gpu_id: Optional[int] = Form(default=None),
    return_base64: bool = Form(default=False),
) -> Dict[str, Any]:
    start = time.time()
    target_gpu_id = DEFAULT_GPU_ID if gpu_id is None else gpu_id
    target_device = f"cuda:{target_gpu_id}"

    with _infer_lock:
        if _state["device"] != target_device:
            _load_models(target_device)
    ...
    if return_base64:
        resp["result_image_base64"] = _image_to_base64(result_img)
    return resp
